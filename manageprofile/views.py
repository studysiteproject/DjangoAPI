...
class UploadImage(APIView):  # 프로필 이미지 업로드
    ...
    def post(self, request):
        ...
        if (filename[-1]).lower() not in [".png", ".jpg"]:
            msg = {
                "state": "fail",
                "message": "invaild file name. check file name please.",
            }
        ...
        s3_resource.Bucket(self.AWS_BUCKET_NAME).put_object(
            Key=profile_img_name, Body=file, ContentType="image/png", ACL="public-read"
        )

        msg = {
            "state": "success",
            "message": "profile image upload successed.",
...
class AddUrl(APIView):
    def get(self, request):
        ...
        # Url 입력 값 규칙 확인
        if verify_user_url(input_url) is False:
        ...
        Userurl.objects.create(user_id=user_obj, url=input_url)
...
class AddTech(APIView):
    def get(self, request):
        ...
        if isNumber(tech_index) is False:
            msg = {"state": "fail", "detail": "tech_index is not conform to the rule"}
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)

        # 존재하지 않는 기술을 추가하는지 확인
        if isObjectExists(Technologylist, id=tech_index) is False:
        ...
        # 사용자 기술스택에 추가한다.
        Usertechlist.objects.create(user_id=user_obj, tech_id=tech_obj)
...
class AddFavorite(APIView):
    def get(self, request):
        ...
        if isObjectExists(UserFavorite, user_id=user_index, study_id=study_id):
            msg = {
                "state": "fail",
                "detail": "this study is already in your favorite list",
            }
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)
        ...
        UserFavorite.objects.create(user_id=user_obj, study_id=study_obj)
...
class DeleteFavorite(APIView):
    def get(self, request):
        ...
        if isObjectExists(UserFavorite, user_id=user_index, study_id=study_id) is False:
            msg = {"state": "fail", "detail": "invalid study. check study_id please"}
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)
