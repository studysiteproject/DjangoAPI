...
class CreateOrViewComments(APIView):  # 새로운 댓글 추가 & 전체 확인
    def get(self, request, study_id):
        ...
        if access_token is not None and user_index is not None:
    ...
    def post(self, request, study_id):
        ...
        try:
            ...
            msg = {
                "state": "fail",
                "detail": "study_id that does not exist. check please study_id",
            }
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)
...
class CreateReplyComment(APIView):
    ...
    def post(self, request, study_id, comment_id):
        ...
        try:
            ...
            if parent_comment_obj.comment_class is not False:
                msg = {
                    "state": "fail",
                    "detail": "reply comment can only be written by parents comment",
                }
                return Response(msg, status=status.HTTP_400_BAD_REQUEST)
        ...
        # 해당 댓글이 존재하지 않을 경우
            ...
            msg = {
                "state": "fail",
                "detail": "parent comment does not exist. check parent comment please",
            }
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)
...
class UpdateVisibleComment(APIView):
    ...
    def put(self, request, study_id, comment_id):
        ...
        if visible.upper() not in ("TRUE", "FALSE"):
            msg = {
                "state": "fail",
                "detail": "flag is not conform to the rule. choose True or False",
            }
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)

        # 현재 댓글의 가시여부를 수정하려는 사용자가 스터디의 생성자인지 확인한다.
        # 해당 스터디의 생성자가 현재 로그인한 유저인지 확인
        if not isObjectExists(Study, id=study_id, user_id=user_index):
            msg = {
                "state": "fail",
                "detail": "Only the user who created this study can change the visible flag of the comment.",
            }
        ...
        try:
            ...
            msg = {
                "state": "fail",
                "detail": "comment does not exist. check comment please.",
            }
            return Response(msg, status=status.HTTP_400_BAD_REQUEST)
