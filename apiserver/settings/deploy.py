...
SECRET_FILE_DATA = json.loads(
    json.dumps(
        yaml.load(my_bucket.Object("secret/deploy_db_info.yml").get()["Body"].read())
    )
)
DATABASES = SECRET_FILE_DATA

# s3에서 Email 인증 서버에 대한 정보가 저장된 파일을 불러와 설정한다.
SECRET_MAIL_INFO = json.loads(
    my_bucket.Object("secret/auth_mail_info.json").get()["Body"].read()
)
...
CORS_ALLOW_WHITELIST = ["catchstudys.com", "www.catchstudys.com"]
